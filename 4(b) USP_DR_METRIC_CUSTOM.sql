...
ALTER PROCEDURE [dbo].[USP_DR_METRIC_CUSTOM] @CustList VARCHAR(50)
	/*Above will be taken from outside cursor */
AS
/*******************************************
    ** FILE: 4. Gather Debit Metrics Competitor
    ** NAME: USP_DR_METRIC_CUSTOM
    ** DESC: 
    **
...
SET @lv_Status = ''
				...
				where PERIOD=@CustList and PROC_NAME='USP_DR_METRIC_CUSTOM'
				
				IF @lv_Status<>'C' 
					Begin 	
	EXEC  ('DELETE FROM DBO.Performance_Monitoring WHERE  PROC_NAME=''USP_DR_METRIC_CUSTOM'' AND PERIOD='+@CustList+';
		   INSERT INTO DBO.Performance_Monitoring
		   SELECT '+@CustList+', ''USP_DR_METRIC_CUSTOM'', CURRENT_TIMESTAMP, NULL,''I''');
...
IF EXISTS
	...
	WHERE Table_Name = ''Debit_'+@Merchant+'_' + @competitor + '_Aggregate_ActivitySegment_' + @version + '''
	)


DROP TABLE Dosh.dbo.Debit_'+@Merchant+'_' + @competitor + '_Aggregate_ActivitySegment_' + @version + '


create table Dosh.dbo.Debit_'+@Merchant+'_' + @competitor + '_Aggregate_ActivitySegment_' + @version + '(
merchant  Varchar(25),
...
INSERT INTO Dosh.dbo.Debit_'+@Merchant+'_' + @competitor + '_Aggregate_ActivitySegment_' + @version + '
SELECT ''' + @Competitor + ''' AS Merchant,  Z.TransactionDate, Z.Pop,z.Campaign_Zip_Flag,z.SEGMENTFLAG,''0'' AS REDEEMFLAG,''0'' AS TotalCustomers,
[Dosh Debit SIG Spend],[Dosh Debit PIN Spend], [Dosh Debit SIG Transactions],  [Dosh Debit PIN Transactions], [Dosh Debit SIG SHoppers], [Dosh Debit PIN Shoppers]
 FROM (
		...
		where a.BankID in (400,410,411,421,424,429,439,440,479,489)   and e.pop in (''Linked Without Transactions'')  and OnlineFlag IN ' + 
				@OnlineFlag + '
		group by a.TransactionDate,Campaign_Zip_Flag, SEGMENTFLAG  ) z
 left join(
		...
		inner join Dosh.dbo.Customer_ActivitySegment_' + @Merchant + '_' + @Version + 
				' E
		ON B.ARGUSPERMID=E.ARGUSPERMID
 ...
 LEFT JOIN (
		SELECT a.TransactionDate,''Linked Without Transactions'' AS Pop,Campaign_Zip_Flag, SEGMENTFLAG ,SUM(PositiveSpend+NegativeSpend) AS ''Dosh Debit PIN Spend'', 
		...
		and a.BankID in (400,410,411,421,424,429,439,440,479,489)   and e.pop in (''Linked Without Transactions'')
		group by a.TransactionDate,Campaign_Zip_Flag, SEGMENTFLAG ) b
...
[Dosh Debit SIG Spend],[Dosh Debit PIN Spend], [Dosh Debit SIG Transactions],  [Dosh Debit PIN Transactions], [Dosh Debit SIG SHoppers], [Dosh Debit PIN Shoppers]
 ...
 ON z.TransactionDate = a.TransactionDate and z.SEGMENTFLAG=a.segmentflag and z.pop = a.pop AND z.Campaign_Zip_Flag = a.Campaign_Zip_Flag
 LEFT JOIN (
		SELECT a.TransactionDate,''Linked With Transactions'' AS Pop,Campaign_Zip_Flag, SEGMENTFLAG ,SUM(PositiveSpend+NegativeSpend) AS ''Dosh Debit PIN Spend'', 
		SUM(PositiveTrans+NegativeTrans) AS ''Dosh Debit PIN Transactions'', COUNT(DISTINCT B.ArgusPermID) AS ''Dosh Debit PIN Shoppers''
		FROM ( SELECT * 
			   FROM Dosh.dbo.Trans_Debit_Dosh_' + @Competitor + '_' + @MONTH + '_' + @version + '
			   )a
...
[Dosh Debit SIG Spend],[Dosh Debit PIN Spend], [Dosh Debit SIG Transactions],  [Dosh Debit PIN Transactions], [Dosh Debit SIG SHoppers], [Dosh Debit PIN Shoppers]
 ...
 ON z.TransactionDate = a.TransactionDate and z.SEGMENTFLAG=a.segmentflag and z.pop = a.pop AND z.Campaign_Zip_Flag = a.Campaign_Zip_Flag
 LEFT JOIN (
		SELECT a.TransactionDate,''NonDosh'' AS Pop,Campaign_Zip_Flag, SEGMENTFLAG ,SUM(PositiveSpend+NegativeSpend) AS ''Dosh Debit PIN Spend'', 
		SUM(PositiveTrans+NegativeTrans) AS ''Dosh Debit PIN Transactions'', COUNT(DISTINCT B.ArgusPermID) AS ''Dosh Debit PIN Shoppers''
		FROM  ( SELECT * 
			   FROM Dosh.dbo.Trans_Debit_NonDosh_' + @Competitor + '_' + @MONTH + '_' + @version + '
			   )a
...
'
	...
	END
EXEC ('
CREATE CLUSTERED INDEX IDX ON Dosh.dbo.Debit_'+@Merchant+'_' + @competitor + '_Aggregate_ActivitySegment_' + @version + ' (pop,transactiondate)
ALTER TABLE Dosh.dbo.Debit_'+@Merchant+'_' + @competitor + '_Aggregate_ActivitySegment_' + @version + ' REBUILD PARTITION = ALL WITH (DATA_COMPRESSION = PAGE)
...
--Log the End Time
   ...
   EXEC  (' UPDATE DBO.Performance_Monitoring SET EndTime = CURRENT_TIMESTAMP, STATUS=''C'' WHERE  Period = '+@CustList+'
           AND PROC_NAME = ''USP_DR_METRIC_CUSTOM''');
END