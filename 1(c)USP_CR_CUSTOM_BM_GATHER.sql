...
ALTER PROCEDURE [dbo].[USP_CR_CUSTOM_BM_GATHER] @CustList VARCHAR(50)
AS
/*******************************************
    ** FILE: Gather Credit Merchant Industry 
    ** NAME: USP_CR_CUSTOM_BM_GATHER
    ** DESC: Gather Credit Merchant Industry basis Group of Selective Merchants (For e.g. SamsClubGroup)
    **
    ...
    ** DATE:        AUTHOR:             DESCRIPTION:
	25/03/2019      SHASHI              Added to Rollup Scripts per Merchants for Custom Merchants
    **************************************************/
SET NOCOUNT ON
...
WHERE PERIOD = @CustList
	AND PROC_NAME = 'USP_CR_CUSTOM_BM_GATHER'

IF @lv_Status <> 'C'
BEGIN
	EXEC  (
			'DELETE FROM DBO.Performance_Monitoring WHERE  PROC_NAME=''USP_CR_CUSTOM_BM_GATHER'' AND PERIOD=' + @CustList + ';
		   INSERT INTO DBO.Performance_Monitoring
		   SELECT ' + @CustList + ', ''USP_CR_CUSTOM_BM_GATHER'', CURRENT_TIMESTAMP, NULL,''I'''
			);
...
IF EXISTS
                ...
                WHERE Table_Name = ''Credit_Industry_' + @Competitor + '_' + @version + '''
                )
                
				DROP TABLE Dosh.dbo.Credit_Industry_' + @Competitor + '_' + @version + '

				CREATE TABLE Dosh.dbo.Credit_Industry_' + @Competitor + '_' + @version + '(
				Merchant VARCHAR(50),
                ...
                SELECT * 
                FROM Dosh.INFORMATION_SCHEMA.Tables
                WHERE Table_Name = ''Credit_Industry_' + @Competitor + '_' + @version + '_Step1''
                )
                
				DROP TABLE Dosh.dbo.Credit_Industry_' + @Competitor + '_' + @version + '_Step1
				
				CREATE TABLE Dosh.dbo.Credit_Industry_' + @Competitor + '_' + @version + '_Step1 (
				Merchant VARCHAR(50),
				TransactionDate DATE,
				POP VARCHAR(50),
				[Dosh Credit Spend] NUMERIC(38,4),
		...
		*/
		...
		BEGIN
			EXEC  (
				...
				INSERT INTO Dosh.dbo.Credit_Industry_' + @Competitor + '_' + @version + '_Step1
				SELECT ''' + @Competitor + ''' AS Merchant
				,TransactionDate,POP
				,[Dosh Credit Spend]
				,[Dosh Credit Transactions]

				FROM Dosh.dbo.Credit_Industry_' + @Merchant + '_' + @version + '
...
'
		...
		EXEC  ('
			...
			insert into
			Dosh.dbo.Credit_Industry_' + @Competitor + '_' + @version + '
			select merchant,transactiondate,pop,sum([Dosh Credit Spend]) as [Dosh Credit Spend],sum([Dosh Credit Transactions]) as [Dosh Credit Transactions]
			from
			Dosh.dbo.Credit_Industry_' + @Competitor + '_' + @version + '_step1
	...
	ALTER TABLE Dosh.dbo.Credit_Industry_' + @Competitor + '_' + @version + '  REBUILD PARTITION = ALL WITH (DATA_COMPRESSION = PAGE)
		')
