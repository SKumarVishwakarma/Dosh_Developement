...
ALTER PROCEDURE [dbo].[USP_CR_METRIC_CUSTOM_FS] @CustList VARCHAR(50)
	/*Above will be taken from outside cursor */
AS
/*******************************************
    ** FILE: 4. Gather Credit Metrics Competitor
    ** NAME: USP_CR_METRIC_CUSTOM_FS
    ** DESC: 
    **
...
SET @lv_Status = ''
				...
				where PERIOD=@CustList and PROC_NAME='USP_CR_METRIC_CUSTOM_FS'
				
				IF @lv_Status<>'C' 
					Begin 	
	EXEC  ('DELETE FROM DBO.Performance_Monitoring WHERE  PROC_NAME=''USP_CR_METRIC_CUSTOM_FS'' AND PERIOD='+@CustList+';
		   INSERT INTO DBO.Performance_Monitoring
		   SELECT '+@CustList+', ''USP_CR_METRIC_CUSTOM_FS'', CURRENT_TIMESTAMP, NULL,''I''');
...
--Log the End Time
   ...
   EXEC  (' UPDATE DBO.Performance_Monitoring SET EndTime = CURRENT_TIMESTAMP, STATUS=''C'' WHERE  Period = '+@CustList+'
           AND PROC_NAME = ''USP_CR_METRIC_CUSTOM_FS''');
END