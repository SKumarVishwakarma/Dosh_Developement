...
GO
ALTER PROCEDURE [dbo].[USP_REGION_DRIVER_POST_FS] @CustList VARCHAR(50)
	/*Above will be taken from outside cursor */
AS
/*******************************************s
    ** FILE: 
    ** NAME: USP_REGION_DRIVER_POST_FS
    ** DESC: 
    **
...
SET @lv_Status = ''
				...
				where PERIOD=@CustList and PROC_NAME='USP_REGION_DRIVER_POST_FS'
				
				IF @lv_Status<>'C' 
					Begin
--Log the Start Time
	
	EXEC ('DELETE FROM DBO.Performance_Monitoring WHERE  PROC_NAME=''USP_REGION_DRIVER_POST_FS'' AND PERIOD='+@CustList+';
		   INSERT INTO DBO.Performance_Monitoring
		   SELECT '+@CustList+', ''USP_REGION_DRIVER_POST_FS'', CURRENT_TIMESTAMP, NULL,''I''');
...
DECLARE MerchantCursor CURSOR FOR  --here I need Cursor to Pick Merchants from AllDoshMerchantListNew @Merchant, @version, 
	...
	--and CompetitorDebitSicCode<>'' -- Not to select Custom Merchants and CompetitorCreditSicCode<>''	
*/
DECLARE MerchantCursor CURSOR FOR  --here I need Cursor to Pick Merchants from AllDoshMerchantListNew @Merchant, @version, 
...
EXEC('
			...
			IF EXISTS
				...
				WHERE Table_Name = ''PostFrequencySegment_Regional_'+@Merchant+'_'+@Version+'''
				)

		DROP TABLE dosh.dbo.PostFrequencySegment_Regional_'+@Merchant+'_'+@Version+'

			create table dosh.dbo.PostFrequencySegment_Regional_'+@Merchant+'_'+@Version+'(
...
')
	...
	EXEC('DECLARE PopSeg CURSOR FOR
		SELECT pop,PostFrequency as FrequencySegment
		FROM Dosh.dbo.Customer_PostFrequencySegment_' + @Merchant + '_' + @Version + ' --Dosh.dbo.PP_DunkinDonut_FrequencySegment_Jan19TPS 
		group by pop,PostFrequency
		--order by 1,2 
		')
		
		--gather combination of population & frequency for each merchant for each iteration (with monthly refresh)
		--THIS TABLE WILL BE FROM STEP 3 Dosh.dbo.Customer_PostFrequencySegment_'+@Merchant+'_'+@Version+'
...
BEGIN
		...
		EXEC 		
		('insert into dosh.dbo.PostFrequencySegment_Regional_'+@Merchant+'_'+@Version+'
					select cast('''+@DATE+''' as varchar),'''+@pop+''',1 as Campaign_Zip_Flag,'''+@FrequencySegment+'''
					
		insert into dosh.dbo.PostFrequencySegment_Regional_'+@Merchant+'_'+@Version+'
					select cast('''+@DATE+''' as varchar),'''+@pop+''',0 as Campaign_Zip_Flag,'''+@FrequencySegment+'''
					')
...
EXEC ('
ALTER TABLE dosh.dbo.PostFrequencySegment_Regional_'+@Merchant+'_'+@Version+' REBUILD PARTITION = ALL WITH (DATA_COMPRESSION = PAGE)
')
...
--Log the End Time
   ...
   EXEC (' UPDATE DBO.Performance_Monitoring SET EndTime = CURRENT_TIMESTAMP, STATUS=''C'' WHERE  Period = '+@CustList+'
           AND PROC_NAME = ''USP_REGION_DRIVER_POST_FS''');
END