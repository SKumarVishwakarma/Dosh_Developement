...
GO
ALTER PROCEDURE [dbo].[USP_NATION_DRIVER_POST_FS] @CustList VARCHAR(50)
	/*Above will be taken from outside cursor */
AS
/*******************************************s
    ** FILE: 
    ** NAME: USP_NATION_DRIVER_POST_FS
    ** DESC: 
    **
...
SET @lv_Status = ''
				...
				where PERIOD=@CustList and PROC_NAME='USP_NATION_DRIVER_POST_FS'
				
				IF @lv_Status<>'C' 
					Begin
--Log the Start Time
	
	EXEC  ('DELETE FROM DBO.Performance_Monitoring WHERE  PROC_NAME=''USP_NATION_DRIVER_POST_FS'' AND PERIOD='+@CustList+';
		   INSERT INTO DBO.Performance_Monitoring
		   SELECT '+@CustList+', ''USP_NATION_DRIVER_POST_FS'', CURRENT_TIMESTAMP, NULL,''I''');
...
EXEC ('
			...
			IF EXISTS
				...
				WHERE Table_Name = ''PostFrequencySegment_National_'+@Merchant+'_'+@Version+'''
				)

		DROP TABLE dosh.dbo.PostFrequencySegment_National_'+@Merchant+'_'+@Version+'

			create table dosh.dbo.PostFrequencySegment_National_'+@Merchant+'_'+@Version+'(
...
BEGIN
		...
		EXEC  		
		('insert into dosh.dbo.PostFrequencySegment_National_'+@Merchant+'_'+@Version+'
...
EXEC ('
ALTER TABLE dosh.dbo.PostFrequencySegment_National_'+@Merchant+'_'+@Version+' REBUILD PARTITION = ALL WITH (DATA_COMPRESSION = PAGE)
')
...
--Log the End Time
   ...
   EXEC  (' UPDATE DBO.Performance_Monitoring SET EndTime = CURRENT_TIMESTAMP, STATUS=''C'' WHERE  Period = '+@CustList+'
           AND PROC_NAME = ''USP_NATION_DRIVER_POST_FS''');
END