...
ALTER PROCEDURE [dbo].[USP_CR_METRIC_CUSTOM_POST_FS] @CustList VARCHAR(50)
	/*Above will be taken from outside cursor */
AS
/*******************************************
    ** FILE: 
    ** NAME: USP_CR_METRIC_CUSTOM_POST_FS
    ** DESC: 
    **
...
SET @lv_Status = ''
				...
				where PERIOD=@CustList and PROC_NAME='USP_CR_METRIC_CUSTOM_POST_FS'
				
				IF @lv_Status<>'C' 
					Begin 	
	EXEC  ('DELETE FROM DBO.Performance_Monitoring WHERE  PROC_NAME=''USP_CR_METRIC_CUSTOM_POST_FS'' AND PERIOD='+@CustList+';
		   INSERT INTO DBO.Performance_Monitoring
		   SELECT '+@CustList+', ''USP_CR_METRIC_CUSTOM_POST_FS'', CURRENT_TIMESTAMP, NULL,''I''');
...
FROM Dosh.dbo.AllDoshMerchantListNew where Is_Active=1 and Competitor<>'' and CampaignStartDate is not NULL and CompetitorCreditSicCode=''   --WHERE CompetitorIndexNumber IN (208) Removed to Include all Competitor
*/
DECLARE MerchantCursor CURSOR
FOR
...
IF EXISTS
	...
	WHERE Table_Name = ''Credit_'+@Merchant+'_' + @competitor + '_Aggregate_PostFrequencySegment_' + @version + '''
	)


DROP TABLE Dosh.dbo.Credit_'+@Merchant+'_' + @competitor + '_Aggregate_PostFrequencySegment_' + @version + '


create table dosh.[dbo].Credit_'+@Merchant+'_' + @competitor + '_Aggregate_PostFrequencySegment_' + @version + '(
merchant  Varchar(25),
...
INSERT INTO Dosh.dbo.Credit_'+@Merchant+'_' + @competitor + '_Aggregate_PostFrequencySegment_' + @version + '
SELECT ''' + @competitor + ''' as merchant,a.TransactionDate,''Linked Without Transactions'' AS Pop,Campaign_Zip_Flag, PostFrequency, ''0'' AS REDEEMFLAG,  '''' as TotalCustomers,
...
FROM Dosh.dbo.Trans_Credit_Dosh_' + @Competitor + '_' + @MONTH + '_' + @version + ' a
	...
	inner join Dosh.dbo.Customer_PostFrequencySegment_' + 
	...
	group by a.TransactionDate, PostFrequency ,Campaign_Zip_Flag 
	--order by 3,2,5
	

--1) Linked With Transactions
INSERT INTO Dosh.dbo.Credit_'+@Merchant+'_' + @competitor + '_Aggregate_PostFrequencySegment_' + @version + '
SELECT ''' + @competitor + '''as merchant,a.TransactionDate,''Linked With Transactions'' AS Pop,Campaign_Zip_Flag, PostFrequency, ''0'' AS REDEEMFLAG,  '''' as TotalCustomers,
SUM(PositiveSpend+NegativeSpend) AS [Dosh Credit Spend], SUM(PositiveTrans+NegativeTrans) AS [Dosh Credit Transactions], COUNT(DISTINCT B.ArgusPermID) AS [Dosh Credit Shoppers]
FROM Dosh.dbo.Trans_Credit_Dosh_' + @Competitor + '_' + @MONTH + '_' + @version + '  a
	INNER JOIN Dosh.dbo.YG_DOSH_PermIdTradeIdAcctId_Yearly_' + @CustList + '_Open_Deduped b with (nolock)
	ON a.AccountID = b.AccountId
...
--3) NonDosh
INSERT INTO Dosh.dbo.Credit_'+@Merchant+'_' + @competitor + '_Aggregate_PostFrequencySegment_' + @version + '
SELECT ''' + @competitor + ''' as merchant,a.TransactionDate,''NonDosh'' AS Pop,Campaign_Zip_Flag, PostFrequency, ''0'' AS REDEEMFLAG,  '''' as TotalCustomers,
SUM(PositiveSpend+NegativeSpend) AS [Dosh Credit Spend], SUM(PositiveTrans+NegativeTrans) AS [Dosh Credit Transactions], COUNT(DISTINCT B.ArgusPermID) AS [Dosh Credit Shoppers]
...
'
	...
	END
EXEC ('
CREATE CLUSTERED INDEX IDX ON Dosh.dbo.Credit_'+@Merchant+'_' + @competitor + '_Aggregate_PostFrequencySegment_' + @version + ' (pop,transactiondate)
ALTER TABLE Dosh.dbo.Credit_'+@Merchant+'_' + @competitor + '_Aggregate_PostFrequencySegment_' + @version + ' REBUILD PARTITION = ALL WITH (DATA_COMPRESSION = PAGE)
...
--Log the End Time
   ...
   EXEC  (' UPDATE DBO.Performance_Monitoring SET EndTime = CURRENT_TIMESTAMP, STATUS=''C'' WHERE  Period = '+@CustList+'
           AND PROC_NAME = ''USP_CR_METRIC_CUSTOM_POST_FS''');
END