...
GO
ALTER PROCEDURE [dbo].[USP_REGION_DRIVER_FS] @CustList VARCHAR(50)
	/*Above will be taken from outside cursor */
AS
/*******************************************s
    ** FILE: Regional Frequency Report Driver Table 
    ** NAME: USP_REGION_DRIVER_FS
    ** DESC: 
    **
...
SET @lv_Status = ''
				...
				where PERIOD=@CustList and PROC_NAME='USP_REGION_DRIVER_FS'
				
				IF @lv_Status<>'C' 
					Begin
--Log the Start Time
	
	EXEC ('DELETE FROM DBO.Performance_Monitoring WHERE  PROC_NAME=''USP_REGION_DRIVER_FS'' AND PERIOD='+@CustList+';
		   INSERT INTO DBO.Performance_Monitoring
		   SELECT '+@CustList+', ''USP_REGION_DRIVER_FS'', CURRENT_TIMESTAMP, NULL,''I''');
...
EXEC('
			...
			IF EXISTS
				...
				WHERE Table_Name = ''FrequencySegment_Regional_'+@Merchant+'_'+@Version+'''
				)

		DROP TABLE dosh.dbo.FrequencySegment_Regional_'+@Merchant+'_'+@Version+'

			create table dosh.dbo.FrequencySegment_Regional_'+@Merchant+'_'+@Version+'(
...
BEGIN
		...
		EXEC 		
		('insert into dosh.dbo.FrequencySegment_Regional_'+@Merchant+'_'+@Version+'
...
EXEC ('
ALTER TABLE dosh.dbo.FrequencySegment_Regional_'+@Merchant+'_'+@Version+' REBUILD PARTITION = ALL WITH (DATA_COMPRESSION = PAGE)
')
...
--Log the End Time
   ...
   EXEC (' UPDATE DBO.Performance_Monitoring SET EndTime = CURRENT_TIMESTAMP, STATUS=''C'' WHERE  Period = '+@CustList+'
           AND PROC_NAME = ''USP_REGION_DRIVER_FS''');
END